...
Como administrador del sistema del Admin Console, una de sus primeras tareas es definir y configurar un sistema de identidades con el que se autenticarán los usuarios finales. A medida que su organización adquiera licencias de Learning Manager, deberá aprovisionar dichas licencias a los usuarios finales. Para ello, necesitará una forma de autenticar a estos usuarios. Realice el siguiente procedimiento para configurar el SSO para sus usuarios.
...
1. En la ****[!UICONTROL URL de autenticación iniciada por IDP]**** , introduzca la URL de autenticación proporcionada por su proveedor de servicios.
...
Como administrador, cuando su empresa adquiere Learning Manager, uno de los pasos cruciales que debe realizar es la migración. Es fundamental que mueva el contenido de formación y los datos de usuario existentes a Learning Manager. El siguiente flujo de trabajo de migración te ayuda a aprovechar las ventajas del LMS moderno e intuitivo sin perder ninguno de los datos heredados de tu organización.
...
Para realizar el flujo de trabajo de migración, necesita los privilegios del administrador de integración. Como administrador, puede asumir la función de administrador de integración o asignarla a otro usuario.
...
1. Configurar carpetas de FTP y BOX para la migración
...
Antes de comenzar el proceso de migración, debe cumplir los siguientes requisitos previos:
...
Después de evaluar los datos existentes, debe asignarlos con las especificaciones de CSV estándar en Learning Manager. Descargue el siguiente ejemplo ***csv-specifications.zip*** que contiene siete hojas de excel necesarias para esta migración. Estas hojas de Excel contienen especificaciones con descripciones para que comprenda cómo asignar los datos existentes a los campos de los archivos .csv.

<!--
<Download link to the zip file>
-->

Asegúrese de que cada archivo .csv contenga los datos de cada campo en el formato prescrito:
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>Mencione un nombre de autor para un curso determinado, ya que a veces varios nombres de autor no se muestran con precisión en la aplicación después de la migración. </p></td> 
  </tr> 
  <tr> 
   <td><p>3</p></td> 
   <td><p>module_version.xlsx </p></td> 
   <td><p>Metadatos para module_version.csv</p></td> 
   <td><p>Asegúrese de indicar la ruta URL de la carpeta de la cuenta de Box donde ha cargado el contenido. </p></td> 
  </tr> 
  <tr> 
   <td><p>4</p></td> 
  ...
  <tr> 
   ...
   <td><p>Proporcione los datos de registros de alumno necesarios en el archivo .csv aunque no sean obligatorios. Sin esta información, aunque el archivo .csv se procese para la migración, es posible que la aplicación Learning Manager no refleje ningún dato. </p></td> 
  </tr> 
 </tbody> 
</table>
...
* learning_program_instance.csv depende del programa de aprendizaje y learning_program_course.csv
* learning_program_course.csv depende de learning_program.csv.
* learning_program_enrollment.csv depende del programa de aprendizaje y learning_program_instance.csv
...
Después de exportar los datos, guarde los archivos .csv en el equipo local. Los archivos ya están listos para colocarse en las carpetas de FTP o BOX.

## Configurar carpetas de FTP y BOX para la migración {#setupftpandboxfoldersforthemigration}

Antes de planificar e iniciar la migración real de todo el contenido, debe configurar las carpetas FTP y BOX. Estas carpetas son necesarias para colocar los archivos .csv en ellas. Una vez que el contenido heredado, en forma de archivos .csv, esté disponible en las carpetas FTP y BOX, Learning Manager podrá utilizar los datos.

### Configurar una cuenta FTP {#setupanftpaccount}
...
Vea una captura de pantalla de ejemplo de los archivos de proyecto y la carpeta de ExaVault, como se muestra aquí:
...
## Configurar una cuenta de BOX {#setupaboxaccount}

Para crear una cuenta de BOX y configurar una carpeta de BOX, realice los siguientes pasos:
...
En la ****[!UICONTROL Introducir correo electrónico]**** , introduzca el ID de correo electrónico donde desea recibir las instrucciones de inicio de sesión para conectarse a Box.

Haga clic en **[!UICONTROL ** Connect **.]**

Recibiría un correo electrónico de Box con un vínculo a la carpeta compartida. Si no tiene una cuenta de Box, haga clic en Registrarse y cree una cuenta. A continuación, las instrucciones de inicio de sesión se envían al ID de correo electrónico del administrador de integración.
...
* No se puede deshacer una ejecución que ya está en proceso. Sin embargo, puede utilizar la opción de eliminación existente en cada función de Learning Manager para deshacer cualquier migración de datos o contenido.
...
En la página de inicio de la migración, el sistema muestra los proyectos de migración que ya se han creado en su organización.
...
En la ****[!UICONTROL Crear un nuevo proyecto de migración]**** , especifique el nombre del proyecto.

![](assets/migrating-the-content-1.png)

Especifique una etiqueta para el proyecto, el catálogo del curso y proporcione una descripción para el proyecto de migración. Los elementos de datos de migración se identifican mediante la etiqueta del proyecto de migración. Si no tiene ningún catálogo de cursos específico y elige el catálogo predeterminado en la lista desplegable, todos los cursos que migre utilizando un proyecto de migración se incluirán en el catálogo que elija en esta fase. Si no elige ningún catálogo, todos los cursos migrados formarán parte del catálogo predeterminado.

Haga clic en **[!UICONTROL Crea.]**
...
***Antes de marcar el proyecto de migración como completado, asegúrese de que todos los sprints del proyecto se hayan completado. Una vez marcado el proyecto de migración como completado, no puede volver atrás y crear sprints en ese proyecto. No puede realizar ninguna modificación en ese proyecto. Solo puede crear otro proyecto de migración y agregarle sprints.***

Después de migrar los datos y el contenido de aprendizaje desde el LMS heredado de su organización, verifique si los datos y el contenido se importan correctamente. Puede verificarlo iniciando sesión como administrador y comprobando la disponibilidad de los datos y el contenido de los módulos y cursos importados

Para obtener recursos útiles sobre la migración, consulte lo siguiente:

* Solución de problemas de migración
* Preguntas frecuentes sobre la carga de CSV
